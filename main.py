...
#***********************PÁGINAS DO SISTEMA*********************** 
        ...
        self.btn_outro.clicked.connect(lambda: self.Pages.setCurrentWidget(self.pg_import))
    ...
    def subscribe_user(self):
        ...
        if self.txt_senha1.text() != self.txt_senha2.text():
            msg = QMessageBox()
            msg.setIcon(QMessageBox.Warning)
            msg.setWindowTitle("Senhas divergentes")
            msg.setText("Favor digitar a mesma senha")
            msg.exec_()
            return None

        nome = self.txt_nome.text()
        user = self.txt_usuario.text()
        password = self.txt_senha1.text()
    ...
    def question(self, table):
        ...
        if table == 'estorno':
            msgBox.setText("Deseja estornar as notas selecionados?")
            msgBox.setInformativeText("As notas selecionadasvoltarão para o estoque \n clique em 'yes' para confirmar!")
